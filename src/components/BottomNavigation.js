...
export default function BottomNavigation() {
  ...
  // Masquer sur desktop et tablets
  if (!isMobile) {
    return null
  }
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`touch-target-large flex flex-col items-center justify-center px-2 py-2 rounded-xl transition-all duration-200 active:scale-95 ${
                    active
